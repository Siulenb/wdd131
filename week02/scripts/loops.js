...
// Function to populate favorite foods using .forEach()
// function populateFavoriteFoods() {
...
// Function to populate favorite foods using .map()
function populateFavoriteFoods() {
  const favoriteFoodsList = document.querySelector("#favorite-foods");
  const foodItems = myInfo.favoriteFoods.map(food => {
    const li = document.createElement("li");
    li.textContent = food;
    return li;
  });
  foodItems.forEach(item => favoriteFoodsList.appendChild(item));
}
